...
def bucket_fsdp_all_gather_with_plan(
    ...
    for bucket_id, ag_bucket in enumerate(all_gather_bucket_plan):
        ...
        for ag_info, ag_snodes in ag_bucket.items():
            if len(ag_snodes) == 0:
                continue
            example_ag_fx_node = _get_fx_node(
                ag_snodes[0],
    ...
    for snode in snodes:
        if (
            ...
            assert (
                snode in ag_snode_to_bucket_id
...
def bucket_fsdp_reduce_scatter_with_plan(
    sched: "scheduler.Scheduler",
    snodes: list["scheduler.BaseSchedulerNode"],
    name_to_buf: Dict[str, "scheduler.SchedulerBuffer"],
    name_to_fused_node: Dict[str, "scheduler.BaseSchedulerNode"],
    reduce_scatter_bucket_plan: list[
        Dict[tuple[Any, ...], list["scheduler.BaseSchedulerNode"]]
    ],
    bucketable_nodes: set[str],
) -> list["scheduler.BaseSchedulerNode"]:
    ...
    for snode in snodes:
        if is_collective(
            ...
            assert (
                len(rs_snode.outputs[0].users) == 1
    ...
    for bucket_id, rs_bucket in enumerate(reduce_scatter_bucket_plan):
        ...
        for rs_info, rs_snodes in rs_bucket.items():
            if len(rs_snodes) == 0:
                continue
            example_rs_fx_node = _get_fx_node(
                rs_snodes[0],
    ...
    for snode in snodes:
        if snode not in rs_and_its_recursive_users:
            ...
            assert (
                snode in rs_snode_to_bucket_id
